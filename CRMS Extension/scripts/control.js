...
// Code for the Smart Scan default setting:
chrome.storage.local.get(["smartScan"]).then((result) => {
  console.log(result.smartScan);
  if (result.smartScan){
  var selectedOption = "input[name='smartscan'][value='" + result.smartScan + "']";
    document.querySelector(selectedOption).checked = true;
  }
});

document.querySelectorAll('input[name="smartscan"]').forEach(function(radio) {
  radio.addEventListener('change', function() {
    console.log(this.value);

    chrome.storage.local.set({ "smartScan": this.value }).then(() => {
       console.log("Smart Scan setting changed");
       console.log(this.value);
       chrome.runtime.sendMessage("smartScan");
     });
...
// Code for the Container Check default setting:
chrome.storage.local.get(["containerCheck"]).then((result) => {
  console.log(result.containerCheck);
  if (result.containerCheck){
  var selectedOption = "input[name='containercheck'][value='" + result.containerCheck + "']";
    document.querySelector(selectedOption).checked = true;
  }
});

document.querySelectorAll('input[name="containercheck"]').forEach(function(radio) {
  radio.addEventListener('change', function() {
    console.log(this.value);

    chrome.storage.local.set({ "containerCheck": this.value }).then(() => {
       console.log("Container Check setting changed");
       console.log(this.value);
       chrome.runtime.sendMessage("containerCheck");
     });
...
async function getCachedLatestTag() {
  ...
  if (latestReleaseTag){
    ...
    if (liveVersion !== latestVersion){
      const helperVersionLine = document.getElementById("maifest-version");
